...
export async function POST(req: Request) {
  try {
    ...
    /* ================= Rate limiting ================= */
    const { allowed } = rateLimit(`scan:${ipAddress}`);
      ...
      return NextResponse.json(
        {
          ...
          message: "You can only perform 5 scans per minute. Please wait.",
        },
        { status: 429 }
      );
    }
    ...
    return NextResponse.json(
      {
        ...
        message: "Something went wrong while processing the scan.",
      },
      { status: 500 }
    );
  }
